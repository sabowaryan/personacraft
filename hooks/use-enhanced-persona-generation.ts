...
// Types pour l'état enrichi de génération
...
export interface UseEnhancedPersonaGenerationReturn {
...
export function useEnhancedPersonaGeneration(): UseEnhancedPersonaGenerationReturn {
  ...
  const comparePersonas = useCallback((personaIds: string[]): PersonaComparison => {
    ...
    return {
      ...
      quality_comparison: selectedPersonas.reduce((acc, persona) => ({
        ...acc,
        [persona.id]: {
          completeness: persona.validation_metrics.completeness_score,
          consistency: persona.validation_metrics.consistency_score,
          realism: persona.validation_metrics.realism_score
        }
      }), {})
    };
  }, [personas]);
  ...
  // Helpers
  const updateGenerationState = (updates: Partial<EnhancedGenerationState>) => {
    setGenerationState(prev => ({ ...prev, ...updates }));
  };

  const calculateQualityTrends = (personas: EnhancedPersona[]) => {
    ...
    return {
      average_completeness: personas.reduce((sum, p) => sum + p.validation_metrics.completeness_score, 0) / personas.length,
      average_consistency: personas.reduce((sum, p) => sum + p.validation_metrics.consistency_score, 0) / personas.length,
      average_realism: personas.reduce((sum, p) => sum + p.validation_metrics.realism_score, 0) / personas.length
    };
  };
