...
export class RealTimeMonitor extends EventEmitter {
    ...
    private calculateCacheHitRate(): number {
        // Get actual cache stats from optimized cache
        try {
            const { optimizedCache } = require('../cache/optimized-cache');
            const cacheStats = optimizedCache.getStats();
            return cacheStats.hitRate;
        } catch (error) {
            console.warn('Could not get cache stats for monitoring:', error);
            return 0; // Return 0 if cache stats unavailable
        }
    }
